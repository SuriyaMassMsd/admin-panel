...
export default function ApprovedCourses({ data, handleClick }) {
  if (!data.length) return <div className="px-16">No Courses Available</div>;
        ...
        <Grid item xs={1} sm={4} md={4} key={index}>
          <Item onClick={() => handleClick(item)}>
            <div className="relative cursor-pointer flex flex-col justify-between h-[280px]">
              <span className="absolute -top-3 -right-3 px-3 py-1.5 rounded bg-[#0ead69] text-white font-semibold">
                Approved
              </span>
              <img
